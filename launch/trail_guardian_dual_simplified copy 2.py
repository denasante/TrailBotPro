...
def generate_launch_description():
    ...
    nav2_launch_arg = DeclareLaunchArgument(
        'nav2', default_value='False', description='Flag to launch Nav2 for Parrot'
    )
    nav2_husky_arg = DeclareLaunchArgument(
        'nav2_husky', default_value='False', description='Flag to launch Nav2 for Husky'
    )
    ...
    slam_parrot_arg = DeclareLaunchArgument(
        'slam_parrot', default_value='False', description='Flag to launch SLAM for Parrot'
    )

    slam_husky_arg = DeclareLaunchArgument(
        'slam_husky', default_value='False', description='Flag to launch SLAM for Husky'
    )
    ...
    ld.add_action(nav2_husky_arg)
    ld.add_action(slam_parrot_arg)
    ld.add_action(start_husky_arg)
    ld.add_action(start_drone_arg)
    ld.add_action(slam_husky_arg)
    ...
    parrot_scan_relay = Node(
        package='41068_ignition_bringup',
        executable='frame_id_relay.py',
        name='parrot_scan_frame_relay',
        parameters=[{
            'input_topic': '/parrot/scan/raw',
            'output_topic': '/parrot/scan',
            'message_type': 'sensor_msgs/msg/LaserScan',
            'frame_id': 'parrot/base_scan',
        }],
        condition=IfCondition(LaunchConfiguration('start_drone'))
    )
    ...
    husky_slam = Node(
        ...
        condition=IfCondition(LaunchConfiguration('slam_husky'))
    )
    ...
    parrot_slam = Node(
        ...
        condition=IfCondition(LaunchConfiguration('slam_parrot'))
    )
    ld.add_action(parrot_slam)



    # =========================================================================
    # NAV2 - PARROT (Aerial Navigation)
    # =========================================================================
    # Using direct node launches for proper namespace support
    # Based on nav2_bringup/navigation_launch.py but adapted for multi-robot
    
    
    parrot_controller = Node(
        package='nav2_controller',
        executable='controller_server',
        namespace='parrot',
        output='screen',
        parameters=[
            PathJoinSubstitution([config_path, 'parrot_nav2_params.yaml']),
            {'use_sim_time': use_sim_time}
        ],
        remappings=[
            ('/tf', 'tf'),
    ...
    ld.add_action(parrot_controller)
    ld.add_action(parrot_planner)
    ld.add_action(parrot_behavior)
    ld.add_action(parrot_bt_navigator)
    ld.add_action(parrot_waypoint_follower)
    ld.add_action(parrot_velocity_smoother)
    ld.add_action(parrot_lifecycle_manager)

    # =========================================================================
    # NAV2 - HUSKY (Ground Navigation)
    # =========================================================================
    # Using direct node launches for proper namespace support
    
    husky_controller = Node(
        package='nav2_controller',
        executable='controller_server',
        namespace='husky',
        output='screen',
        parameters=[
            PathJoinSubstitution([config_path, 'husky_nav2_params.yaml']),
            {'use_sim_time': use_sim_time}
        ],
        remappings=[
            ('/tf', 'tf'),
        ...
        condition=IfCondition(LaunchConfiguration('nav2_husky'))
    )
    ...
    ld.add_action(husky_controller)
    ld.add_action(husky_planner)
    ld.add_action(husky_behavior)
    ld.add_action(husky_bt_navigator)
    ld.add_action(husky_waypoint_follower)
    ld.add_action(husky_velocity_smoother)
    ld.add_action(husky_lifecycle_manager)
